...
class AppointmentScheduleController extends Controller
{
    ...
    public function saveAppointmentSchedule(CreateRequest $request)
    {
        ...
        // Ensure 'commission_type' exists in general settings
        if (isset($general_settings['commission_type']) && $general_settings['commission_type'] == 'commission_base') {
        ...
        if ($request->is_schedule_required) {
            $records = AppointmentSchedule::where('law_firm_id', $law_firm_id)
            ...
            foreach ($request->selected_days as $day) {
                ...
                foreach ($request->generated_slots[$day] as $slot) {
            ...
            AppointmentSchedule::where('law_firm_id', $law_firm_id)
                               ->where('appointment_type_id', $request->appointment_type_id)
                               ->delete();
            if ($request->fee) {
    ...
    public function getAppointmentSchedules(Request $request)
    {
        ...
        if ($request->is_schedule_required) {
            $schedules = AppointmentSchedule::withAll()
                                            ->where('law_firm_id', $user->law_firm->id)
                                            ->where('appointment_type_id', $request->appointment_type_id)
                                            ->get();
            ...
            $schedule = AppointmentSchedule::withAll()
                                           ->where('law_firm_id', $user->law_firm->id)
                                           ->where('appointment_type_id', $request->appointment_type_id)
                                           ->first();
            if ($schedule) {
    ...
    public function addNewAppointmentSchedule(AddScheduleRequest $request)
    {
        ...
        foreach ($request->generated_slots as $slot) {
            AppointmentScheduleSlot::create([

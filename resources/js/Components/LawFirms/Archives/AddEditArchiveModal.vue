<template>
    ...
    <Modal maxWidth="lg" :id="id">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ modalData ? __('update'):__('create') }} {{__('archive')}}</h5>
                ...
                <div class="row">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li v-for="(lang, index) in $page.props.translation_languages" :key="lang.id" class="nav-item" role="presentation">
                            <button :class="{'active': index == 0}" class="nav-link mr-1" :id="`archive-nav-${lang.code}-tab`" data-bs-toggle="tab" :data-bs-target="`#archive-nav-${lang.code}`" type="button" role="tab" :aria-controls="`archive-nav-${lang.code}`" aria-selected="true">{{ lang.name }}</button>
                        </li>
                    </ul>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select v-model="form.archive_category_id" class="form-select"
                                aria-label="category">
...
export default defineComponent({
    ...
    methods:{
        ...
        create(){
            this.form.post(this.route('law_firms.law_firm_archives.store'), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
        ...
        update(){
            this.form.transform((data) => ({
                    ...data,
                    _method: "PUT",
                })).post(this.route('law_firms.law_firm_archives.update',{law_firm_archive:this.modalData.id}), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {

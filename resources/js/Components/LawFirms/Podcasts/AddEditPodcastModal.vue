<template>
    ...
    <Modal maxWidth="lg" :id="id">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ modalData ? __('update'):__('create') }} {{__('podcast')}}</h5>
                ...
                <div class="row">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li v-for="(lang, index) in $page.props.translation_languages" :key="lang.id" class="nav-item" role="presentation">
                            <button :class="{'active': index == 0}" class="nav-link mr-1" :id="`podcasts-nav-${lang.code}-tab`" data-bs-toggle="tab" :data-bs-target="`#podcasts-nav-${lang.code}`" type="button" role="tab" :aria-controls="`podcasts-nav-${lang.code}`" aria-selected="true">{{ lang.name }}</button>
                        </li>
                    </ul>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select v-model="form.podcast_category_id" class="form-select"
                                aria-label="category">
                    ...
                    <div class="col-md-6">
                        ...
                        <div v-if="form.link_type =='internal' && form.file_type == 'audio'" class="form-group mb-3">
                            ...
                            <input id="law_firm-audio" @change="onPodcastFileChange($event,'audio')" type="file" />
                        ...
                        <div v-if="form.link_type =='internal' && form.file_type == 'video'" class="form-group mb-3">
                            ...
                            <input id="law_firm-video" @change="onPodcastFileChange($event,'video')" type="file" />
...
export default defineComponent({
    ...
    methods:{
        ...
        create(){
            this.form.post(this.route('law_firms.law_firm_podcasts.store'), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
        ...
        update(){
            this.form.transform((data) => ({
                    ...data,
                    _method: "PUT",
                })).post(this.route('law_firms.law_firm_podcasts.update',{law_firm_podcast:this.modalData.id}), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {

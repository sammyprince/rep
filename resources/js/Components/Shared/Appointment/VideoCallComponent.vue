<template>
  ...
  <div class="row" v-if="$page.props.auth.logged_in_as == 'lawyer' || $page.props.auth.logged_in_as == 'law_firm'">
    ...
    <div class="col-md-10 d-flex justify-content-end flex-wrap">
      <button type="button" @click="joinEvent" v-if="fetched && (!audioPermissionMessage || !videoPermissionMessage) && appointment.appointment_status_code == 2" :disabled="disableJoin" class="btn btn-primary px-5">
         {{ __('Make Call') }}
      </button>

      <button type="button" @click="leaveEvent" v-if="disableJoin" class="btn btn-danger ms-lg-3 ms-0  px-5" plain :disabled="!disableJoin">
        {{ __('leave call') }}
      </button>
        ...
        .then(() => {
          this.rtc
            ...
            .then((stream) => {
              ...
              if (
                this.$page.props.auth.logged_in_as == "lawyer" ||
                this.$page.props.auth.logged_in_as == "law_firm"
              ) {
  ...
  async created() {
    ...
    Echo.private(`make-agora-call.${this.appointment.id}`).listen(
      ...
      (e) => {
        console.log(e);
        if (
          this.$page.props.auth.logged_in_as == "customer" &&
          this.$page.props.auth.user.customer.id == e.customer_id
        ) {

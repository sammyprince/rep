<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_events.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="event in lawyer_events.data" :key="event.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_events.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditEventModal from "@/Components/Lawyers/Events/AddEditEventModal.vue";
import ViewEventModal from "@/Components/Lawyers/Events/ViewEventModal.vue";
import DeleteEventModal from "@/Components/Lawyers/Events/DeleteEventModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerEvents(loading_more)
     },
     clearModalData(){
        this.modal_event = null
        this.key++
     },
    getLawyerEvents(loading_more){
    axios.post(this.route('lawyers.lawyer_events.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_events.data = this.lawyer_events.data.concat(data.data);
        }else{
            this.lawyer_events.data = data.data;
        }
        this.lawyer_events.links = data.links
        this.lawyer_events.meta = data.meta
        this.fetching = false
    });
    },
  }

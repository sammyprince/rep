<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_posts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="post in lawyer_posts.data" :key="post.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_posts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditPostModal from "@/Components/Lawyers/Posts/AddEditPostModal.vue";
import ViewPostModal from "@/Components/Lawyers/Posts/ViewPostModal.vue";
import DeletePostModal from "@/Components/Lawyers/Posts/DeletePostModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerPosts(loading_more)
     },
     clearModalData(){
        this.modal_post = null
        this.key++
     },
    getLawyerPosts(loading_more){
    axios.post(this.route('lawyers.lawyer_posts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_posts.data = this.lawyer_posts.data.concat(data.data);
        }else{
            this.lawyer_posts.data = data.data;
        }
        this.lawyer_posts.links = data.links
        this.lawyer_posts.meta = data.meta
        this.fetching = false
    });
    },
  }

<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_podcasts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="podcast in lawyer_podcasts.data" :key="podcast.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_podcasts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditPodcastModal from "@/Components/Lawyers/Podcasts/AddEditPodcastModal.vue";
import ViewPodcastModal from "@/Components/Lawyers/Podcasts/ViewPodcastModal.vue";
import DeletePodcastModal from "@/Components/Lawyers/Podcasts/DeletePodcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerPodcasts(loading_more)
     },
     clearModalData(){
        this.modal_podcast = null
        this.key++
     },
    getLawyerPodcasts(loading_more){
    axios.post(this.route('lawyers.lawyer_podcasts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_podcasts.data = this.lawyer_podcasts.data.concat(data.data);
        }else{
            this.lawyer_podcasts.data = data.data;
        }
        this.lawyer_podcasts.links = data.links
        this.lawyer_podcasts.meta = data.meta
        this.fetching = false
    });
    },
  }

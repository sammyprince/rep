<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_educations.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="education in lawyer_educations.data" :key="education.id">
...
<!--            <img v-if="education.image" :src="education.image" width="75" height="75" :alt="education.image" />-->
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_educations.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditEducationModal from "@/Components/Lawyers/Educations/AddEditEducationModal.vue";
import ViewEducationModal from "@/Components/Lawyers/Educations/ViewEducationModal.vue";
import DeleteEducationModal from "@/Components/Lawyers/Educations/DeleteEducationModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerEducations(loading_more)
     },
     clearModalData(){
        this.modal_education = null
        this.key++
     },
    getLawyerEducations(loading_more){
    axios.post(this.route('lawyers.lawyer_educations.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_educations.data = this.lawyer_educations.data.concat(data.data);
        }else{
            this.lawyer_educations.data = data.data;
        }
        this.lawyer_educations.links = data.links
        this.lawyer_educations.meta = data.meta
        this.fetching = false
    });
    },
  }

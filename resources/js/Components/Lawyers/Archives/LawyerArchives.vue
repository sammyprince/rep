<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_archives.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="archive in lawyer_archives.data" :key="archive.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_archives.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditArchiveModal from "@/Components/Lawyers/Archives/AddEditArchiveModal.vue";
import ViewArchiveModal from "@/Components/Lawyers/Archives/ViewArchiveModal.vue";
import DeleteArchiveModal from "@/Components/Lawyers/Archives/DeleteArchiveModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerArchives(loading_more)
     },
     clearModalData(){
        this.modal_archive = null
        this.key++
     },
    getLawyerArchives(loading_more){
    axios.post(this.route('lawyers.lawyer_archives.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_archives.data = this.lawyer_archives.data.concat(data.data);
        }else{
            this.lawyer_archives.data = data.data;
        }
        this.lawyer_archives.links = data.links
        this.lawyer_archives.meta = data.meta
        this.fetching = false
    });
    },
  }

<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_broadcasts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="broadcast in lawyer_broadcasts.data" :key="broadcast.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_broadcasts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditBroadcastModal from "@/Components/Lawyers/Broadcasts/AddEditBroadcastModal.vue";
import ViewBroadcastModal from "@/Components/Lawyers/Broadcasts/ViewBroadcastModal.vue";
import DeleteBroadcastModal from "@/Components/Lawyers/Broadcasts/DeleteBroadcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerBroadcasts(loading_more)
     },
     clearModalData(){
        this.modal_broadcast = null
        this.key++
     },
    getLawyerBroadcasts(loading_more){
    axios.post(this.route('lawyers.lawyer_broadcasts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_broadcasts.data = this.lawyer_broadcasts.data.concat(data.data);
        }else{
            this.lawyer_broadcasts.data = data.data;
        }
        this.lawyer_broadcasts.links = data.links
        this.lawyer_broadcasts.meta = data.meta
        this.fetching = false
    });
    },
  }
